...
std::vector<MPI_Request> async_dd_sendrecv_rvec(const struct gmx_domdec_t gmx_unused *dd,
	...
	rvec gmx_unused *buf_r, int gmx_unused n_r, int tag)
{
#if GMX_MPI
	int        rank_s, rank_r;
	MPI_Status stat;
	...
	{
		MPI_Request req;
		MPI_Isend(buf_s[0], n_s * sizeof(rvec), MPI_BYTE, rank_s, tag,
			dd->mpi_comm_all, &req);
	...
	{
		MPI_Request req;
		MPI_Irecv(buf_r[0], n_r * sizeof(rvec), MPI_BYTE, rank_r, tag,
			dd->mpi_comm_all, &req);
		requests.push_back(req);
	}
	return requests;
